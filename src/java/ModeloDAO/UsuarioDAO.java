...
public class UsuarioDAO extends ConexionBd implements Crud {
    ...
    public boolean inactivarEstado(String estadoInhabilitado, String idUsu) {
        try {
            ...
            puente = conexion.prepareStatement(sql);
            puente.setString(1, estadoInhabilitado);
            puente.setString(2, idUsu);
            puente.executeUpdate();
            operacion = true;
    ...
    public boolean Estado(String estadoActivo, String usuaId) {
        try {
            sql = "update usuario set ESTA_USUA=? where ID_USUA =?";
            puente = conexion.prepareStatement(sql);
            puente.setString(1, estadoActivo);
            puente.setString(2, usuaId);
            puente.executeUpdate();
            operacion = true;
